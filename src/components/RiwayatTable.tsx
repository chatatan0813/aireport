          ...
          <tbody>
            {filtered.map((r, i) => (
              <tr key={i} className="border-b border-slate-800/60">
                ...
                <td className="tabular py-2 pr-4 text-slate-300">Rp {formatRupiah(r.omzet)}</td>
                <td className={`tabular py-2 pr-4 ${r.laba !== null && r.laba < 0 ? "text-rose-400" : "text-slate-300"}`}>
                  {r.laba === null ? "—" : "Rp " + formatRupiah(r.laba)}
                </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="py-6 text-center text-slate-500">Belum ada data.</td>
              </tr>
            )}
          </tbody>
        </table>
